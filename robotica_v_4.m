...
Small changes by Aaron T. Becker Mohammad Sultan in 2017
...
*)
(*drawing functions*)
drawZArrow[jr_]:=Line[{{{0,0,0},{0,0,2jr}}, (*mathematica's built-in 3D arrow sometimes draws poorly*)
...
drawJoint[ j_,d_,r_,\[Theta]_]:=Module[{jr = 1/5,ar = 1/20,pr=1/7,vr=1/6},{
...
(*draw joint*)
{LightBlue,If[j == "prismatic",Cuboid[{-pr,-pr,-pr},{pr,pr,+pr}],
{Cylinder[{{0,0,-jr},{0,0,+jr}},vr]
}]},
(*draw x(i+1) *)
...
drawGripper[g_,r_]:=Module[{jr = 1/5,ar = 1/20},{
(*draw z-axis*)
drawCoordAxes[jr],
If[r!= 0,
{Gray,Cuboid[{-2ar,-ar,-4ar},{0,ar,4ar}],
...
If[jointtype[1]== "revolute",drawJoint[jointtype[1],d[1],a[1],params[[1]]],drawJoint[jointtype[1],params[[1]],a[1],theta[1]]],

If[dof==1,GeometricTransformation[drawGripper[g,0],Chop[Td[dof]]],
If[showRobot,
 Table[If[jointtype[i]=="revolute",GeometricTransformation[drawJoint[jointtype[i],d[i],a[i],params[[i]]],Td[i-1]],GeometricTransformation[drawJoint[jointtype[i],params[[i]],a[i],theta[i]],Td[i-1]]],{i,2,dof}]],
If[jointtype[dof]== "revolute",GeometricTransformation[drawShaft[ jointtype[dof],d[dof], a[dof],params[[dof]]],Td[dof-1]],GeometricTransformation[drawShaft[ jointtype[dof],params[[dof]], a[dof],theta[dof]],Chop[Td[dof-1]]]]],
GeometricTransformation[drawGripper[g,0],Chop[Td[dof]]],
Text[StringForm["\!\(\*
...
{{showRobot,True,"show robot"},{True,False}},ControlPlacement->Left,
...
Manipulate[Module[{jr=1/10,ar=1/40,Ad,Td,Ts,j,i,Jval,U,\[CapitalSigma]full,V,\[CapitalSigma],o0,on,Jvalw,Uw,\[CapitalSigma]w,Vw},
...
,{Uw,on}]],
(*ground*){LightBrown,Cylinder[{{0,0,-2/5},{0,0,-1/5-1/20}},2.2]},If[jointtype[1]=="revolute",drawJoint[jointtype[1],d[1],a[1],params[[1]]],drawJoint[jointtype[1],params[[1]],a[1],theta[1]]],If[showRobot,Table[If[jointtype[i]=="revolute",GeometricTransformation[drawJoint[jointtype[i],d[i],a[i],params[[i]]],Td[i-1]],GeometricTransformation[drawJoint[jointtype[i],params[[i]],a[i],theta[i]],Td[i-1]]],{i,2,dof}]],If[jointtype[dof]=="revolute",GeometricTransformation[drawShaft[jointtype[dof],d[dof],a[dof],params[[dof]]],Td[dof-1]],GeometricTransformation[drawShaft[jointtype[dof],params[[dof]],a[dof],theta[dof]],Td[dof-1]]],GeometricTransformation[drawGripper[g,0],Td[dof]],Text[StringForm["\!\(\*
StyleBox[\"H\",\nFontSlant->\"Italic\"]\)=``",MatrixForm[N[Td[dof],2]]],{0,0,-3.2}]
(*,Text[StringForm["J=``",MatrixForm[N[Jval,2]]],{0,0,2.2}]*)


},SphericalRegion->True,ImageSize->425,Boxed->False]],{{params,ConstantArray[0,dof]},ControlType->None},Dynamic[Grid[Table[With[{i=i},If[jointtype[i]=="prismatic",{Subscript["d",i],Slider[Dynamic[params[[i]]],{0,1,1/20},ImageSize->Small],Dynamic[params[[i]]]},{Subscript["\[Theta]",i],Slider[Dynamic[params[[i]]],{-\[Pi],\[Pi],\[Pi]/32},ImageSize->Small],Dynamic[params[[i]]]}]],{i,dof}](*Table*)]],(*Dynamic*)Delimiter,{{g,1,"grip"},0,1,0.01,ImageSize->Small,Appearance->"Labeled"},{{showRobot,True,"show robot"},{True,False}},
{{showJacobian,True,"show Jv"},{True,False}},
{{showJacobianw,True,"show J\[Omega]"},{True,False}},
