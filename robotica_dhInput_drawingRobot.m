...
Small changes by Aaron T. Becker in 2017
...
If[NumberQ[x2] && x2>0, dof = x2; createdh[],If[ Length[x2]==5 && Length[x3]==2 , dof= x3[[2]]; DH = x2; createdh2[],dof= Input["You entered wrong Matrix.
...
*)
FKin[]:=
	Do[If[$dhInput$ == "YES", Print["DH Parameter entered from dhInput[]"], If[$DATAFILE$ == "YES",Print["DH Parameter entered from DataFile[]"],   
...
DrawJoint[ j_,d_,r_,\[Theta]_]:=Module[{jr = 1/5,ar = 1/20},{
...
(*draw joint*)
{LightBlue,If[j == "prismatic",Cuboid[{-jr,-jr,-jr},{jr,jr,+jr}],
{Cylinder[{{0,0,-jr},{0,0,+jr}},jr]
}]},
(*draw x(i+1) *)
...
drawRobot[]:=
Manipulate[
Module[{jr = 1/10,ar = 1/40,Ad,Td,Ts,j,i},
Ad =Table[ 
If[jointtype[i]=="prismatic",
...
If[jointtype[dof]== "revolute",GeometricTransformation[DrawShaft[ jointtype[dof],d[dof], a[dof],params[[dof]]],Td[dof-1]],GeometricTransformation[DrawShaft[ jointtype[dof],params[[dof]], a[dof],theta[dof]],Td[dof-1]]],
